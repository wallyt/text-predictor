...
3. Experiment with lemmatization, which is similar to stemming but takes grammatical context into account.
4. Construct predictive models that look at the last one, two and three words you typed (using Markov chains for one-, two- and three-gram models) stored in data tables. To handle word combinations that weren't in the training corpus, the models will implement Kneser-Way smoothing.
5. Refactor the model and code to ensure it's as efficient as possible, measuring the accuracy (using perplexity as the metric) and finding the right balance of performance, runtime and size.
6. Build a Shiny web app that uses the efficient model to predict the next word you need as you type.
