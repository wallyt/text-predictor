...
4. Construct n-gram models using one, two and three words, using Markov chains. The models will implement zero generalization to handle word combinations that weren't in the training corpus, such as with backoff models or smoothing. At this stage I will measure the accuracy and performance of the model using 
5. Refactor the model and code to ensure it's as efficient as possible.
6. Build a Shiny app that uses the model to predict the next word you need as you type.
